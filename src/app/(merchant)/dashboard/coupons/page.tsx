...
export default function CouponsPage() {
  ...
  return (
    <div className="max-w-xl space-y-6">
      ...
      <form onSubmit={create} className="bg-card rounded-[20px] border border-border p-4 flex gap-2 shadow-sm">
        ...
        <button className="px-6 bg-ink text-white rounded-xl font-bold hover:bg-ink-hover transition-colors text-sm">إضافة</button>
      </form>
        ...
        <div className="divide-y divide-border">
          {list.map(c=>(
            <div key={c.id} className="p-4 flex justify-between items-center hover:bg-card-hover/50 transition-colors">
              ...
              <button onClick={()=>toggle(c)} className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${c.is_active ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-500/20" : "bg-background border-border text-muted"}`}>{c.is_active ? "نشط" : "موقف"}</button>
            </div>
          ))}
