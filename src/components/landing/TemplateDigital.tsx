...
export default function TemplateDigital({ product, merchant, config }: { product: Product; merchant: Merchant; config: Config }) {
  ...
  const titleWords = (config.hero_title || product.name || "NOVA DIGITAL").split(" ");
  const t1 = titleWords[0] || "NOVA";
  const t2 = titleWords.slice(1).join(" ") || "DIGITAL";
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section id="order" className="max-w-[1280px] mx-auto border-x border-white/20 grid lg:grid-cols-2">
        <div className="p-6 lg:p-8 border-b lg:border-b-0 lg:border-r border-white/10">
          ...
          <p className="text-sm leading-relaxed opacity-60 mt-4">{product.description || "Software premium avec licence officielle, installation automatique, support 24/7."}</p>
          <div className="mt-6 grid gap-px bg-white/10 border border-white/10">
            <div className="flex justify-between p-3 bg-[#0B0B0C] font-mono text-xs"><span className="opacity-40">Version</span><span>2.5.0</span></div>
            <div className="flex justify-between p-3 bg-[#0B0B0C] font-mono text-xs"><span className="opacity-40">متوافق مع</span><span>Windows 10 / 11, macOS</span></div>
            <div className="flex justify-between p-3 bg-[#0B0B0C] font-mono text-xs"><span className="opacity-40">الدعم</span><span>24/7 عبر الدردشة</span></div>
          </div>
          <div className="mt-6 space-y-2">
            {[{"q": "تفعيل", "a": "ترسل المفتاح عبر البريد الإلكتروني خلال دقائق."}, {"q": "ضمان", "a": "ضمان 12 شهر لاستعادة المبلغ."}].map((f, i) => (
        ...
        <div className="bg-white text-[#111] p-6 lg:p-8">
          <h3 className="font-bold">ترخيص الاستخدام</h3>
          <p className="text-xs opacity-60 mt-1">رخيصة officielle — للدعم الفني: 24/7</p>
          <div className="mt-6">
            {/* License key display and download form would go here */}
            <button className="w-full bg-primary text-white py-2 rounded-lg font-medium hover:opacity-90 transition-opacity">
              {config.show_download_link ? `Download ${config.download_text || ""}` : "عرض تفاصيل الترخيص"}
            </button>
          </div>
        </div>
      </section>

      <footer className="border-t border-white/10 py-6 text-center text-xs font-mono opacity-40">© {merchant.business_name} • NOVA DIGITAL — ORDELY</footer>
    </div>
  );
}